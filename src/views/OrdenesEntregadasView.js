...
function OrdenesEntregadasView(props) {
    const {
        ordenes,
        setOrdenes,
        selectedOrden,
    ...
    return (
      ...
      <h1>Lista de Órdenes Entregadas</h1>
      <ul>
        {ordenes.map((orden) => (
          <li key={orden.id}>
            ...
            <button className="botonImagen2" onClick={() => handleNavigate(`/OrdenDelivered/${orden.numeroOrden}`)}>
                Seleccionar Orden
            </button>
          </li>
        ))}
...
export default OrdenesEntregadasView;