...
import DetallesOrdenDeliveredView from '../views/DeliveredOrderView';
...
function DetallesOrdenDelivered() {
  const { numeroOrden } = useParams();
  const [orden, setOrden] = useState(null);
  const [productos, setProductos] = useState([]);
  const [total, setTotal] = useState(0);
...
export {CerrarCompra,OrdenesPendientes,OrdenesEntregadas,ListaOrdenes,DetallesOrden,IngresarDireccion,Carrito,FinalizarCompra,DetallesOrdenAdmin,OrdenesConfirmadas,DeliveryMenu,DetallesOrdenDelivery,DetallesOrdenDelivered};
